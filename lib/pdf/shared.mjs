...
const renderCellContent = (cell, col, header = false) => {
	const textStyle = [
	...
	if (Array.isArray(cell)) {
		...
		return [
			h(Text, { style: textStyle }, primary == null || primary === '' ? '-' : String(primary)),
			secondary
				? h(Text, { style: styles.tdMuted }, String(secondary))
				: null,
		];
	}
	return h(
...
const DataTable = ({ columns, rows, footer, note }) =>
	h(
		View,
		null,
		h(
			...
			h(
				...
				columns.map((col, index) =>
					h(
						...
						renderCellContent(col.label, col, true)
					)
				)
			),
						...
						return h(
							View,
							{
								...
								style: [
									styles.tdWrap,
									{ width: col.width },
									cellIndex === columns.length - 1 ? styles.cellLast : {},
									col.align === 'right' ? { alignItems: 'flex-end' } : {},
								],
							},
							renderCellContent(cell, col)
						);
					})
				)
			),
			footer
				? h(
						View,
						{ style: styles.tableFooter, wrap: false },
						footer.map((cell, cellIndex) => {
							...
							return h(
								...
								renderCellContent(cell, { ...col, strong: true })
							);
						})
				  )
				: null
